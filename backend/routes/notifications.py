...
@router.get("/")
def list_notifications(unread_only: bool = False, limit: int = 50, db: Session = Depends(get_db), engineer=Depends(get_current_engineer)):
    q = db.query(models.Notification).filter(models.Notification.engineer_name == engineer.name).order_by(models.Notification.created_at.desc())
    if unread_only:
...
def unread_count(db: Session = Depends(get_db), engineer=Depends(get_current_engineer)):
    count = db.query(models.Notification).filter(
        models.Notification.engineer_name == engineer.name,
